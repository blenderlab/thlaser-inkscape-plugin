...
#            p = np[:]        
    ...
    def effect_bitmap(self, selected):
        ...
        for node in self.document.getroot().xpath(path, namespaces=inkex.NSS): 
            x = float(node.get('x','0')) * self.unitScale
            y = float(node.get('y','0')) * self.unitScale
            w = float(node.get('width','0'))
            h = float(node.get('height','0'))
            datauri = node.get(inkex.addNS('href','xlink')) 
            
            gcode += ";image at %f/%f mm, size %f/%f\n" % (x,y,w * self.unitScale,h * self.unitScale)
            gcode += "G0 X%f Y%f F%i\nG0 F%i\n" % (x,y,self.options.movefeed,self.options.bitmapfeed)

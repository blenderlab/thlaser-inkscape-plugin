...
def get_chunks(arr, chunk_size=32):
...
LASER_SPEED = "G0 F%d ;speed\n"
TOOL_CHANGE = "T%02d ;(select tool)\nM117 tool change\n\n"

HEADER_TEXT = ';(Generated by thlaser v%s inkscape plugin)\n;(based on code from gcode_tools)\n\n\nG90 ;absolute coords\n' % VERSION
HOMING_TEXT = 'G28 X0 Y0\n'
FANON_TEXT = 'M106 S255\n'
FANOFF_TEXT = 'M107\n'
FOOTER_TEXT = "%s\n;(end)\n" % LASER_OFF
...
def draw_svg_rect(x, y, w, h, parent):
    rect_attribs = {
        'style': simplestyle.formatStyle({'stroke': 'none', 'stroke-width': '1', 'fill': 'red', 'fill-opacity': '0.1'}),
...
class Gcode_tools(inkex.Effect):
    def __init__(self):
        ...
        self.OptionParser.add_option("-y", "--Yoffset", action="store", type="float", dest="Yoffset", default="0.0",
                                     help="Offset along Y")
        self.OptionParser.add_option("-p", "--feed", action="store", type="float", dest="feed", default="1000",
                                     help="Cutting feed rate in unit/min")
        self.OptionParser.add_option("-m", "--movefeed", action="store", type="float", dest="movefeed", default="6000",
                                     help="Movement feed rate in unit/min")
        self.OptionParser.add_option("-b", "--bitmapfeed", action="store", type="float", dest="bitmapfeed",
                                     default="1000", help="Bitmap feed rate in unit/min")

        self.OptionParser.add_option("", "--pixelspermm", action="store", type="int", dest="pixelspermm", default="10",
                                     help="Pixels per mm")
        ...
        self.OptionParser.add_option("", "--function", action="store", type="string", dest="function", default="Curve",
                                     help="What to do: Curve|Area|Area inkscape")
        self.OptionParser.add_option("", "--tab", action="store", type="string", dest="tab", default="",
                                     help="Means nothing right now. Notebooks Tab.")

        self.OptionParser.add_option("", "--mirrory", action="store", type="inkbool", dest="mirrory", default=False,
                                     help="set laser coord system zero to bottom left (like inkscape)")
        self.OptionParser.add_option("", "--dovector", action="store", type="inkbool", dest="dovector", default=False,
                                     help="generate gcode for vector")
        self.OptionParser.add_option("", "--dobitmap", action="store", type="inkbool", dest="dobitmap", default=False,
                                     help="generate gcode for bitmap")
        self.OptionParser.add_option("", "--invbitmap", action="store", type="inkbool", dest="invbitmap", default=False,
                                     help="invert bitmap")

        self.OptionParser.add_option("", "--double_sided_cutting", action="store", type="inkbool",
                                     dest="double_sided_cutting", default=False,
                                     help="Generate code for double-sided cutting.")
        self.OptionParser.add_option("", "--draw-curves", action="store", type="inkbool", dest="drawCurves",
                                     default=False, help="Draws curves to show what geometry was processed")
        self.OptionParser.add_option("", "--logging", action="store", type="inkbool", dest="logging", default=False,
                                     help="Enable output logging from the plugin")
        self.OptionParser.add_option("", "--usespindle", action="store", type="inkbool", dest="usespindle",
                                     default=False, help="Use M3/M5 for laser control (instead of G1)")
        self.OptionParser.add_option("", "--homing", action="store", type="inkbool", dest="homing", default=False,
                                     help="Home XY on start")
        self.OptionParser.add_option("", "--airfan", action="store", type="inkbool", dest="airfan", default=False,
                                     help="Turn on fan while cutting")
        ...
        # new options for bitmap
        self.OptionParser.add_option("", "--bitmap_mode", action="store", type="int", dest="bitmap_mode", default="0",
                                     help="")
        self.OptionParser.add_option("", "--modulation_type", action="store", type="int", dest="modulation_type",
                                     default="1",
                                     help="")
        self.OptionParser.add_option("", "--scale_type", action="store", type="int", dest="scale_type", default="1",
                                     help="")
        self.OptionParser.add_option("", "--bw_threshold", action="store", type="int", dest="bw_threshold",
                                     default="128", help="")
        self.OptionParser.add_option("", "--b_threshold", action="store", type="int", dest="b_threshold",
                                     default="210", help="")
        self.OptionParser.add_option("", "--w_threshold", action="store", type="int", dest="w_threshold",
                                     default="220", help="")
        self.OptionParser.add_option("", "--outputpng", action="store", type="inkbool", dest="outputpng", default=True,
                                     help="")
    ...
    def check_dir(self):
        if (os.path.isdir(self.options.directory)):
            if (os.path.isfile(self.options.directory + '/header')):
                ...
                if self.options.homing:
                    self.header += HOMING_TEXT
                if self.options.airfan:
                    self.header += FANON_TEXT
    ...
    def make_args(self, c, gcmd):
        ...
        m = [self.options.Xscale, self.options.Yscale, 1, self.options.Xscale, self.options.Yscale, 1]
        ...
        for i in range(6):
            if c[i] != None:
                ...
                value = self.unitScale * c[i]
                value += a[i] * self.unitScale
                if (self.options.mirrory):
                    if (i == 1):
                        value = self.pageHeight - value
    ...
    def generate_gcode(self, curve, depth, altfeed=None):
        ...
        for i in range(1, len(curve)):
            ...
            if (lg not in ["G1", "G2", "G3"]):
                feed = f
            else:
                feed = ""

            if s[1] == 'move':
                # Traversals (G0) tend to signal either the toolhead coming up, going down, or indexing to a new workplace.  All other cases seem to signal cutting.
                self.laseron = 0
                if self.options.usespindle:
                    gcode += LASER_OFF + "\n"
                gcode += "G0 " + self.make_args(si[0], lg) + " F" + str(self.options.movefeed) + "\n"
                lg = 'G0'
                ...
                # if lg=="G0":
                # gcode += "G1 " + self.make_args([None,None,s[5][0]+depth]) + feed +"\n"
                # gcode += LASER_ON
                if (not self.laseron):
                    self.laseron = 1
                    if self.options.usespindle:
                        gcode += LASER_ON
                gcode += "G1 " + self.make_args(si[0], lg) + feed + "\n"
                lg = 'G1'
                ...
                if abs((dx ** 2 + dy ** 2) * self.options.Xscale) > self.options.min_arc_radius:
                    ...
                    if abs(r1.mag() - r2.mag()) < 0.001:
                        if (s[3] > 0):
                            ...
                            gcode += ccwArc
                        gcode += " " + self.make_args(si[0] + [None, dx, dy, None], lg) + feed + "\n"
                        ...
                        if (s[3] > 0):
                            ...
                            gcode += ccwArc
                        gcode += " " + self.make_args(si[0], lg) + " R%f" % (r * self.options.Xscale) + feed + "\n"
    ...
    def tool_change(self):
        ...
        gcode = TOOL_CHANGE % (self.currentTool)
        return gcode
    ...
    def effect_bitmap(self, selected):
        ...
        for id, node in self.selected.iteritems():
            if node.tag == inkex.addNS('image', 'svg'):
                ...
                datauri = node.get(inkex.addNS('href', 'xlink'))

                # wtf?
                # inv = 1 / self.unitScale
                #poop = abs((self.pageWidth - self.pageHeight) * inv)
                #gcode += ";poop=%f pw=%f ph=%f inv=%f\n" % (poop, self.pageWidth, self.pageHeight, inv)

                draw_svg_rect(x, y, w, h, self.document.getroot())

                gcode += ";image at %f/%f mm, size %f/%f id %s\n" % (
                x * self.unitScale, y * self.unitScale, w * self.unitScale, h * self.unitScale, node.get('id'))
                gcode += "G0 " + self.make_args([x, y], "G0") + " F" + str(int(self.options.movefeed)) + "\n"
                gcode += "G7 R" + str(self.options.pixelspermm) + " F" + str(int(self.options.bitmapfeed)) + "\n"

                imgstr = re.search(r'base64,(.*)', datauri).group(1)
                tempimg = cStringIO.StringIO(imgstr.decode('base64'))

                image = Image.open(tempimg).convert('RGB')
                ...
                if self.options.bitmap_mode == 0:
                    image = image.convert("L")
                if self.options.bitmap_mode == 1:
                    image = image.convert("L")
                    pixels = list(image.getdata())
                    pixels[:] = [map_values(x,0,255,self.options.b_threshold,self.options.w_threshold) for x in pixels]
                    image.putdata(pixels)
                if self.options.bitmap_mode == 2:
                    image = image.convert("L")
                    pixels = list(image.getdata())
                    pixels[:] = [255 if x >= self.options.bw_threshold else 0 for x in pixels]
                    image.putdata(pixels)
                if self.options.bitmap_mode == 3:
                    image = image.convert("1", dither=Image.FLOYDSTEINBERG)
                if self.options.bitmap_mode == 4:
                    image = image.convert("L")
                    pixels = list(image.getdata())
                    pixels[:] = [255 if x >= self.options.bw_threshold else random.randint(0, 255) for x in pixels]
                    image.putdata(pixels)
                if self.options.bitmap_mode == 5:
                    image = image.convert("L")
                    pixels = list(image.getdata())
                    pixels[:] = [0 if x < self.options.bw_threshold else random.randint(0, 255) for x in pixels]
                    image.putdata(pixels)
                ...
                scale_type = Image.NEAREST
                if self.options.scale_type == 2:
                    scale_type = Image.BILINEAR
                if self.options.scale_type == 3:
                    scale_type = Image.BICUBIC
                if self.options.scale_type == 4:
                    scale_type = Image.ANTIALIAS
                ...
                if scalew != width or scaleh != height:
                    ...
                    image = image.resize((scalew, scaleh), scale_type)

                # end image manipulation

                if self.options.outputpng:
                    pngimage = image.convert('RGB');
                    pngimage.save(self.options.directory + '/' + self.options.file + id + '.png')
                ...
                for row in reversed(pixels):
                    ...
                    for chunk in get_chunks(result_row):
                        if first:
                            ...
                            gcode += "G7 "
                        b64 = ("".join(chr(x) for x in chunk)).encode("hex")
                        gcode += "L" + str(len(b64)) + " "
                        gcode += "D" + b64 + "\n"
                    forward = not forward
    ...
    def effect(self):
        ...
        self.options.Xscale = 1
        self.options.Yscale = -1
        self.laseron = 0
        selected = self.selected.values()

        root = self.document.getroot()
        self.pageHeight = float(parseInt(root.get("height", None)))
        self.pageWidth = float(parseInt(root.get("width", None)))
        ...
        logger.logpath = self.filename + '.log'
...
;0.0  = %f (Offset y)
        ...
        self.procimages = 0
        if self.options.dobitmap:
            gcode += self.effect_bitmap(selected)

        if self.options.dovector:
            gcode += self.effect_curve(selected)

        if (self.options.double_sided_cutting):
            gcode += "\nM0 Please flip over material\n"
            # Include a tool change operation
            gcode += self.tool_change()
        ...
        if (self.skipped - self.procimages != 0):
            inkex.errormsg("Warning: skipped %d object(s) because they were not paths" % self.skipped)
